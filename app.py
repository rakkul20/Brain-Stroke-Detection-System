...
def result():
    ...
    Covid_status = int(request.form['Covid_status'])

    x=np.array([gender,age,hypertension,heart_disease,ever_married,work_type,Residence_type,
                avg_glucose_level,bmi,smoking_status,Covid_status]).reshape(1,-1)
    ...
    # for No Stroke Risk
    if Y_pred==1:
        return render_template('Ischemic_stroke.html')
    elif Y_pred == 2:
        return render_template('subarachnoid_stroke.html')
    elif Y_pred == 3:
        return render_template('Intracerebral_stroke.html')
    else:
